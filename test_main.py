...
SAMPLE_CALIBRATION = {
    'port_map': {
        ...
        '/dev/serial/by-path/board-b': {'1': 'split2', '2': 'finish'},
    },
    'thresholds': {'start': 20, 'split1': 20, 'split2': 20, 'finish': 20},
}
...
def test_build_state_creates_gates_for_every_calibrated_gate():
    ...
    assert set(state.gates.keys()) == {'start', 'split1', 'split2', 'finish'}
...
def test_on_reading_drives_race_engine_through_full_run():
    ...
    on_reading(state, '/dev/serial/by-path/board-b', 2, 35, 1.0)
    on_reading(state, '/dev/serial/by-path/board-b', 2, 10, 1.1)  # finish enter
    on_reading(state, '/dev/serial/by-path/board-b', 2, 35, 1.3)  # finish exit -> completes run
